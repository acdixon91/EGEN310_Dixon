...
uint8_t readPacket(Adafruit_BLE *ble, uint16_t timeout) 
{
  ...
  while (timeout--) {
    ...
    if ((packetbuffer[1] == 'C') && (packetbuffer[replyidx - 1] == '$')){
      leftTrigger = (char)packetbuffer[5]; rightTrigger.concat((char)packetbuffer[6]); rightTrigger.concat((char)packetbuffer[7]);
      rightThumbstick = (char)packetbuffer[11]; leftThumbstick.concat((char)packetbuffer[12]); leftThumbstick.concat((char)packetbuffer[13]);
      break;
    }
